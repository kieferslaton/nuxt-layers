<template>
    <div class="relative -mx-row lg:-mr-row2x lg:ml-0 mb-8">
        <div class="absolute right-full top-0 hidden h-full w-[10vw] flex-col items-center justify-center gap-5 lg:flex">
            <button class="flex h-8 w-8 items-center justify-center rounded-full bg-white"
        ...
        <ClientOnly>
            <Swiper @swiper="onSwiperReady" @slideChange="onSlideChange" :slides-per-view="1.15"
                :space-between="spaceBetween" :slides-offset-before="slidesOffset / 2 - 1"
            ...
            <div class="mt-5 flex w-full justify-center lg:hidden">
                <div class="mx-2 h-[12px] w-[12px] rounded-full" @click="() => swiperRef.slideTo(index)"
                    :class="index === currentSwiperIndex ? 'bg-white' : 'bg-black/30'" v-for="(home, index) in homes"
                    :key="index"></div>
            </div>
        </ClientOnly>
    </div>
