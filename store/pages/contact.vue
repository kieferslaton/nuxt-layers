<template>
    ...
    <div class="flex flex-col gap-8 lg:flex-row">
      <div class="lg:w-1/2 lg:pr-[2.5vw]">
        <div class="relative aspect-video">
          <Map :locations="[{ latitude: latLng.lat, longitude: latLng.lng }]" :zoom="10" />
        </div>
      </div>
      <div class="lg:w-1/2 lg:pl-[2.5vw]">
        <div class="flex flex-col">
          ...
          <span>
            Office:
            <a class="text-secondary underline" :href="`tel:1+${location.phone}`">{{ location.phone }}</a>
          </span>
  ...
  <section class="px-row py-row lg:px-row2x border-t-primary-lighter border-t">
    ...
    <div class="d:grid-cols-3 dl:grid-cols-4 grid gap-8 sm:grid-cols-2">
      <NuxtLink :to="`/agent/${agent.firstName.toLowerCase()}-${agent.lastName.toLowerCase()}`" v-for="agent in agents"
        ...
        <Card>
          ...
          <div v-else class="p-row bg-gray-light mb-4 flex aspect-video w-full justify-center">
          ...
          <div class="flex flex-col items-center px-6 pb-6">
            ...
            <a v-if="agent.email" :href="`mailto:${agent.email}`" class="text-secondary mb-8 underline">{{ agent.email
            }}</a>
            ...
            <a v-if="agent.mobilePhone" :href="`tel:+1${agent.mobilePhone}`"
              ...
              <Icon name="phone" color="white" class="mr-2" size="20" filled />Call Me
            </a>
          </div>
        </Card>
      </NuxtLink>
